...
export class PuntoUbigeoController {
    ...
    createPuntoUbigeo = async (req: Request, res: Response) => {
        const [error, createPuntoUbigeoDto] = CreatePuntoUbigeoDto.create(
            req.body
        );
        if (error) return res.status(400).json({ error });

        this.puntoUbigeoService
            .createPuntoUbigeo(createPuntoUbigeoDto!)
    ...
    updatePuntoUbigeo = async (req: Request, res: Response) => {
        ...
        const [error, updatePuntoUbigeoDto] = UpdatePuntoUbigeoDto.create({
            ...req.body,
            id,
        });
        if (error) return res.status(400).json({ error });
        ...
        this.puntoUbigeoService
            .updatePuntoUbigeo(updatePuntoUbigeoDto!)
    ...
    getPuntoUbigeoByPuntoId = async (req: Request, res: Response) => {
        ...
        this.puntoUbigeoService
            .getPuntoUbigeoByPuntoId(idPuntoContacto)
            .then((puntoUbigeo) => res.status(200).json(puntoUbigeo))
            .catch((error) => this.handleError(res, error));
    };
}
