...
import { CreateAsistenciaDto, CustomError, PaginationDto, UpdateAsistenciaDto } from "../../../domain";
...
export class AsistenciaService {
    ...
    async getAsistenciasByIdCharla(idCharla: number, paginationDto: PaginationDto) {
    ...
    async getAsistenciaById(id: number) {
        ...
        const asistenciaExists = await prisma.asistencia.findUnique({ where: { id } });
        
        if (!asistenciaExists) throw CustomError.badRequest(`Asistencia with id ${id} does not exist`);

        try {
            // Eliminar la asistencia
            const asistenciaEliminada = await prisma.asistencia.delete({ where: { id } });
